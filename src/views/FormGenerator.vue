			...
			<div v-if="!isPreview" class="container">
				...
				<div v-for="(css,index) in allCss" :key="css" class="inputContainer">
					<div class="cssContainer">
						<InputText placeholder="Enter css URL" v-model="css.css" class="p-input" type="text" />
						<Button @click="handleDeleteCss(index)" class="p-cancel">
							<i class="pi pi-minus"></i>
						</Button>
					</div>
				</div>
				<div class="noData" v-if="this.allCss == 0">No Css URL's</div>
			</div>
			<!-- css url's end -->
			<!-- js url's start -->
			<div v-if="!isPreview" class="container">
				...
				<div v-for="(js, ) in allJs" :key="js" class="inputContainer">
					<div class="cssContainer">
						<InputText v-model="js.js" placeholder="Enter js URL" class="p-input" :key="index"
							type="text" />
	...
	methods: {
		...
		handleSubmit() {
			if (
				...
				let existLinks = document.getElementsByTagName('link');
				existLinks = Array.prototype.slice.call(existLinks, 0);
				console.log(existLinks)
				for (let i = 0; i < this.allCss.length; i++) {
					var link = document.createElement("link");
					link.href = this.allCss[i].css;
					const isExists = existLinks.filter((l) => l.href === link.href);
					link.rel = "stylesheet";
					if (!isExists.length) {
				...
				let existScripts = document.getElementsByTagName('script');
				existScripts = Array.prototype.slice.call(existScripts, 0);
				for (let i = 0; i < this.allJs.length; i++) {
					var jsLink = document.createElement("script");
					jsLink.src = this.allJs[i].js;
					jsLink.rel = "script";
					const isExists = existScripts.filter((l) => l.src === jsLink.src);
					if (!isExists.length) {
